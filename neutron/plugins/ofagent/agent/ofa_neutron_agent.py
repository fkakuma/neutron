...
class OFANeutronAgent(n_rpc.RpcCallback,
    ...
    def add_fdb_flow(self, port_info, lvm, ofport):
        datapath = self.tun_br.datapath
        ofp = datapath.ofproto
        ofpp = datapath.ofproto_parser
    ...
    def del_fdb_flow(self, port_info, lvm, ofport):
        datapath = self.tun_br.datapath
        ofp = datapath.ofproto
        ofpp = datapath.ofproto_parser
        if port_info == n_const.FLOODING_ENTRY:
    ...
    def _fdb_chg_ip(self, context, fdb_entries):
        LOG.debug("dummy update chg_ip received")
        pass
    ...
    def setup_tunnel_port(self, agent_ip, network_type):
        port_name = self._create_tunnel_port_name(network_type, agent_ip)
        if not port_name:
            return 0
